...
(() => {
  ...
  const callNumberHitboxes = { main: [], zoom: [], segment: [] };
  ...
  function drawSpectrogram() {
    ...
    if (showCallNumbers && showCallNumbers.checked && lastAutoIdResults && lastAutoIdResults.calls && lastAutoIdResults.calls.length) {
      drawCallNumberOverlay(specCtx, lastAutoIdResults.calls, left, top, plotW, plotH, 0, duration, 'main');
    }
  ...
  function drawCallNumberOverlay(ctx, calls, left, top, plotW, plotH, tMin, tMax, hitboxKey = null) {
    if (!calls || !calls.length) return;
    if (hitboxKey && callNumberHitboxes[hitboxKey]) callNumberHitboxes[hitboxKey] = [];
    const yBox = top + (1 - Math.min(25000, maxFreqDisplay) / maxFreqDisplay) * plotH;
    const boxW = 20;
    const boxH = 17;
    const visible = calls
      .map((call, originalIndex) => ({ call, originalIndex }))
      .filter(item => item.call.t1 >= tMin && item.call.t0 <= tMax);

    visible.forEach(({ call: c, originalIndex }) => {
      ...
      if (hitboxKey && callNumberHitboxes[hitboxKey]) {
        callNumberHitboxes[hitboxKey].push({
          x0: px - boxW / 2 - 4,
          y0: py - boxH / 2 - 4,
          x1: px + boxW / 2 + 4,
          y1: py + boxH / 2 + 4,
      ...
      ctx.fillText(String(originalIndex + 1), px, py + 0.5);
      ctx.textAlign = 'start';
  ...
  function drawSegmentedSpectrogram() {
    ...
    if (showCallNumbers && showCallNumbers.checked && calls.length) {
      drawCallNumberOverlay(segmentCtx, calls, left, top, plotW, plotH, t0, t1, 'segment');
    }
  ...
  function drawZoomSpectrogram() {
    ...
    if (showCallNumbers && showCallNumbers.checked && lastAutoIdResults && lastAutoIdResults.calls && lastAutoIdResults.calls.length) {
      drawCallNumberOverlay(zoomCtx, lastAutoIdResults.calls, left, top, plotW, plotH, t0, t1, 'zoom');
    }
  ...
  function findCallNumberHit(canvas, key, e) {
    const hitboxes = callNumberHitboxes[key] || [];
    if (!hitboxes.length) return null;
    const p = getCanvasPoint(canvas, e);
    return hitboxes.find(h => p.x >= h.x0 && p.x <= h.x1 && p.y >= h.y0 && p.y <= h.y1) || null;
  }

  function ensureCallPopup() {
    if (activeCallPopup) return activeCallPopup;

    const overlay = document.createElement('div');
    overlay.id = 'callPopupOverlay';
    ...
    title.id = 'callPopupTitle';
    title.style.cssText = 'font-size:14px;line-height:1.35;color:#e8edf7;';
    ...
    canvas.id = 'callPopupCanvas';
    canvas.width = 720;
    canvas.height = 390;
    canvas.style.cssText = 'width:100%;height:auto;display:block;border-radius:10px;background:#03050d;';

    const footer = document.createElement('div');
    footer.id = 'callPopupFooter';
    footer.style.cssText = 'margin-top:8px;font-size:12px;color:#aab4c6;line-height:1.35;';
    ...
    document.addEventListener('keydown', ev => {
      if (ev.key === 'Escape' && overlay.style.display !== 'none') hide();
    });

    activeCallPopup = { overlay, title, canvas, footer };
    return activeCallPopup;
  }

  function showCallPopup(call, index) {
    if (!samples || !call) return;
    const popup = ensureCallPopup();
    popup.overlay.style.display = 'flex';
    popup.title.innerHTML = `<b>Call ${index + 1}</b> · ${fmtTime(call.t0)} – ${fmtTime(call.t1)} · picco ${(call.fPeak / 1000).toFixed(1)} kHz`;
    popup.footer.innerHTML = `Durata ${(call.dur * 1000).toFixed(1)} ms · banda ${(call.fMin / 1000).toFixed(1)}–${(call.fMax / 1000).toFixed(1)} kHz · ${escapeHtml(call.suggestion?.label || 'non classificata')}`;
    drawSingleCallSpectrogram(popup.canvas, call, index);
  }

  function drawSingleCallSpectrogram(canvas, call, index) {
    ...
    const callCenter = (call.t0 + call.t1) / 2;
    const callDur = Math.max(0.001, call.t1 - call.t0);
    let win = Math.max(0.040, Math.min(0.180, callDur * 8));
    let t0 = Math.max(0, callCenter - win / 2);
    let t1 = Math.min(duration, callCenter + win / 2);
    if (t1 - t0 < win) {
      if (t0 === 0) t1 = Math.min(duration, win);
      if (t1 === duration) t0 = Math.max(0, duration - win);
    }

    const freqPad = Math.max(10000, (call.fMax - call.fMin) * 0.55);
    let f0 = Math.max(0, call.fMin - freqPad);
    let f1 = Math.min(maxFreqDisplay, call.fMax + freqPad);
    const minFreqSpan = Math.min(maxFreqDisplay, 60000);
    if (f1 - f0 < minFreqSpan) {
      const fc = Math.max(0, Math.min(maxFreqDisplay, call.fPeak || (call.fMin + call.fMax) / 2));
      f0 = Math.max(0, fc - minFreqSpan / 2);
      f1 = Math.min(maxFreqDisplay, fc + minFreqSpan / 2);
      if (f1 - f0 < minFreqSpan) {
        if (f0 === 0) f1 = Math.min(maxFreqDisplay, minFreqSpan);
        if (f1 === maxFreqDisplay) f0 = Math.max(0, maxFreqDisplay - minFreqSpan);
      }
    }

    const left = 70, right = 18, top = 18, bottom = 48;
    const plotW = w - left - right;
    const plotH = h - top - bottom;
    const contrast = Number(contrastSlider.value);
    const minDb = -120 + contrast * 0.25;
    ...
    for (let x = 0; x < plotW; x++) {
      ...
      const s = startSample + frame * hop;
      const frameTime = s / sampleRate;
      const inCall = frameTime >= call.t0 - 0.0015 && frameTime <= call.t1 + 0.0015;
      const re = new Float64Array(n);
      const im = new Float64Array(n);
      for (let i = 0; i < n; i++) re[i] = (samples[s + i] || 0) * window[i];
      fft(re, im);
      ...
      for (let y = 0; y < plotH; y++) {
        const freq = f1 - (y / plotH) * (f1 - f0);
        const bin = Math.min(n / 2 - 1, Math.max(0, Math.floor(freq / (sampleRate / n))));
        const db = 20 * Math.log10(Math.sqrt(re[bin]*re[bin] + im[bin]*im[bin]) + 1e-9);
        let v = (db - minDb) / (maxDb - minDb);
        v = Math.max(0, Math.min(1, Math.pow(v, 0.72)));

        const timeFrac = (frameTime - call.t0) / Math.max(1e-9, call.t1 - call.t0);
        const expected = call.fStart + (call.fEnd - call.fStart) * Math.max(0, Math.min(1, timeFrac));
        const bandPad = Math.max(4500, call.bandwidth * 0.70 + 2500);
        const nearCall = inCall && freq >= call.fMin - 8000 && freq <= call.fMax + 8000 && Math.abs(freq - expected) <= bandPad;
        const c = nearCall ? colorMap(Math.min(1, Math.max(0, (v - 0.12) / 0.88))) : [0, 0, Math.round(18 + v * 34)];
        const idx = (y * plotW + x) * 4;
        img.data[idx] = c[0];
        img.data[idx + 1] = c[1];
        img.data[idx + 2] = c[2];
    ...
    const x0 = left + (call.t0 - t0) / (t1 - t0) * plotW;
    const x1 = left + (call.t1 - t0) / (t1 - t0) * plotW;
    ctx.fillStyle = 'rgba(255,255,255,.08)';
    ctx.fillRect(x0, top, x1 - x0, plotH);
    ctx.strokeStyle = 'rgba(255,255,255,.7)';
    ctx.lineWidth = 1;
    ctx.strokeRect(x0, top, x1 - x0, plotH);
    ...
    ctx.textBaseline = 'middle';
    for (let i = 0; i <= 4; i++) {
      const f = f0 + i / 4 * (f1 - f0);
      const y = top + plotH - i / 4 * plotH;
      ctx.fillText(`${(f / 1000).toFixed(0)} kHz`, left - 8, y);
    ...
    for (let i = 0; i <= 5; i++) {
      ...
      const x = left + i / 5 * plotW;
      ctx.fillStyle = '#d9e1ef';
      ctx.fillText(fmtTime(t), x, top + plotH + 8);
    ...
    ctx.fillText(`Call ${index + 1} · finestra estesa ${fmtTime(t0)} – ${fmtTime(t1)}`, left + 8, top + 20);
  }
  ...
  segmentCanvas.addEventListener('click', (e) => {
    const hit = findCallNumberHit(segmentCanvas, 'segment', e);
    if (!hit) return;
    showCallPopup(hit.call, hit.index);
  });

  segmentCanvas.addEventListener('mousemove', (e) => {
    const hit = findCallNumberHit(segmentCanvas, 'segment', e);
    segmentCanvas.style.cursor = hit ? 'pointer' : 'default';
  });

  // Funziona anche sui numeri disegnati nello spettrogramma principale e nello zoom.
  specCanvas.addEventListener('click', (e) => {
    if (isSelecting) return;
    const hit = findCallNumberHit(specCanvas, 'main', e);
    if (!hit) return;
    showCallPopup(hit.call, hit.index);
  });

  zoomCanvas.addEventListener('click', (e) => {
    const hit = findCallNumberHit(zoomCanvas, 'zoom', e);
    if (!hit) return;
    showCallPopup(hit.call, hit.index);
  });
